...
if __name__ == "__main__":
    ...
    for run in range(1, 11):
        ...
        RESULTS_PATH = paths.BASE_DIR / "artifacts" / "results" / "_steel_plates" / EXPERIMENT_NAME / f"DATA_SPLIT_STATE_{DATA_SPLIT_STATE}" / "test_runs_CE_weighted" 
        MODEL_PATH = paths.BASE_DIR / "artifacts" / "models" / "model_weights" / "_steel_plates" / EXPERIMENT_NAME / f"DATA_SPLIT_STATE_{DATA_SPLIT_STATE}" / "weighted_loss"
        EXPERIMENT_PATH = paths.BASE_DIR / "artifacts" / "results" / "_steel_plates" / EXPERIMENT_NAME / f"DATA_SPLIT_STATE_{DATA_SPLIT_STATE}"
        ...
        sys.stdout = SaveOutput(RESULTS_PATH / f"steel_plates_results_{run}.txt")  # Redirect stdout
        ...
        if os.path.exists(MODEL_PATH / f"steel_plates_test_weights_{run}.pth"):
            print(f"Warning: {RESULTS_PATH} already exists. No new directory will be created.")
        else:            
            MODEL_PATH.mkdir(parents=True, exist_ok=True)
            torch.save(test_net.state_dict(), MODEL_PATH / f"steel_plates_test_weights_{run}.pth")
        ...
        history_df.to_csv(RESULTS_PATH / f"steel_plates_history_CE_{run}.csv", index=False) 
        #df_probs.to_csv(RESULTS_PATH / f"steel_plates_probabilities_CE_{run}.csv", index=False)   

        torch_NN_evaluation.plot_training_metrics(history_df, 
                                    save_path=RESULTS_PATH / f"steel_plates_figures_CE_{run}.png")