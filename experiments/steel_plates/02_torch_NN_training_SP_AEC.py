...
if __name__ == "__main__":
    ...
    for run in range(1, 11):    
        ...
        RESULTS_PATH = paths.BASE_DIR / "artifacts" / "results" / "_steel_plates" / EXPERIMENT_NAME / f"DATA_SPLIT_STATE_{DATA_SPLIT_STATE}" / f"test_runs_AEC_{COST_MATRIX_NAME}" 
        ...
        sys.stdout = SaveOutput(RESULTS_PATH / f"steel_plates_results_AEC_{run}.txt")  # Redirect stdout
        ...
        history_df.to_csv(RESULTS_PATH / f"steel_plates_history_AEC_{run}.csv", index=False) 
        # df_probs.to_csv(RESULTS_PATH / f"steel_plates_probabilities_AEC_{run}.csv", index=False)   

        torch_NN_evaluation.plot_training_metrics(history_df, 
                                    save_path=RESULTS_PATH / f"steel_plates_figures_AEC_{run}.png")