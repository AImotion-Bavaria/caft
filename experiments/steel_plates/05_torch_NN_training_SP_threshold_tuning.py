...
if __name__ == "__main__":
    ...
    for run in range(1, 11):
        ...
        RESULTS_PATH = paths.BASE_DIR / "artifacts" / "results" / "_steel_plates" / EXPERIMENT_NAME / f"DATA_SPLIT_STATE_{DATA_SPLIT_STATE}" / "test_runs_threshold_tuning"
        ...
        # Redirect stdout
        sys.stdout = SaveOutput(RESULTS_PATH / f"steel_plates_results_threshold_tuning_{run}.txt")
        ...
        phase1_history_df.to_csv(
            RESULTS_PATH / f"steel_plates_history_phase1_{run}.csv", index=False)
        torch_NN_evaluation.plot_training_metrics(
            phase1_history_df,
            save_path=RESULTS_PATH / f"steel_plates_figures_phase1_{run}.png",
            Title="Phase 1: Training (Weighted CE)")

        # Threshold search results
        search_df.to_csv(
            RESULTS_PATH / f"steel_plates_threshold_search_{run}.csv", index=False)
        plot_threshold_search(
            search_df, best_tau,
            save_path=RESULTS_PATH / f"steel_plates_figures_threshold_search_{run}.png")

        # Model weights
        weights_dir = MODEL_PATH / "threshold_tuning"
        weights_dir.mkdir(parents=True, exist_ok=True)
        torch.save(model.state_dict(), weights_dir / f"steel_plates_weights_{run}.pth")
        # Save optimal threshold alongside weights
        torch.save({"tau": best_tau}, weights_dir / f"steel_plates_threshold_{run}.pth")
