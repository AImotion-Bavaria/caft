...
if __name__ == "__main__":
    ...
    for run in range(1, 8):
        ...
        RESULTS_PATH = paths.BASE_DIR / "artifacts" / "results" / "_random_blobs_balanced" / EXPERIMENT_NAME / f"DATA_SPLIT_STATE_{DATA_SPLIT_STATE}" / f"test_runs_RWWCE2_{COST_MATRIX_NAME}" 
        MODEL_PATH = paths.BASE_DIR / "artifacts" / "models" / "model_weights" / "_random_blobs_balanced" / EXPERIMENT_NAME / f"DATA_SPLIT_STATE_{DATA_SPLIT_STATE}" / f"RWWCE2_{COST_MATRIX_NAME}" 
        ...
        weight_matrix = torch.tensor([[class_weights[0], 0, 0],
                                       [0, class_weights[1], 0],
                                       [0, 0, class_weights[2]]], dtype=torch.float32).to(device)
