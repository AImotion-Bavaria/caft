...
COST_INCREMENT = 0.75             # How much to increase lambda per cost step
INNER_PATIENCE = 20            # Epochs without FC2-rate improvement before raising cost
MAX_FINETUNE_EPOCHS = 1000      # Total epoch budget for constraint-aware fine-tuning
...
def train_constraint_aware(model, train_loader, val_loader, optimizer, criterion,
    ...
    """
    Stepwise cost-escalation fine-tuning with convergence detection.

    At each cost level the model is trained epoch-by-epoch until:
      (a) the safety constraint is satisfied  → stop entirely, or
      (b) val_loss stagnates for INNER_PATIENCE epochs → increase cost by COST_INCREMENT.

    Overall stopping:
      - Constraint FC2-Rate <= MAX_FP_RATE fulfilled  → stop
      - MAX_FINETUNE_EPOCHS total fine-tuning epochs reached → stop

    Returns:
        model             - fine-tuned model (best weights restored)
        final_lambda      - final cost multiplier value
        final_cost_matrix - cost matrix at the last step
        history_df        - per-epoch metrics
    """
    lambda_safety = COST_INCREMENT  # Start with first cost step (not zero)
    ...
    # Inner patience tracking (FC2-rate stagnation at current cost level)
    best_fc2_at_level = float('inf')
    epochs_without_improvement = 0

    # Per-epoch history
    history = {
        "epoch": [], "cost_step": [], "lambda_safety": [],
        "train_loss": [], "train_accuracy": [],
        "val_loss": [], "val_accuracy": [],
        "val_average_cost": [], "fc2_rate": [],
    }
    ...
    while total_epoch < MAX_FINETUNE_EPOCHS:
        ...
        history["cost_step"].append(cost_step)
        ...
        print(
            f"[Epoch {total_epoch:3d}/{MAX_FINETUNE_EPOCHS} | Cost-Step {cost_step}] "
            f"Loss={train_loss:.4f} | Acc={train_acc:.1f}% | "
            f"ValLoss={val_loss:.4f} | ValAcc={val_acc:.1f}% | "
            f"AvgCost={val_avg_cost:.3f} | "
            f"FC2-Rate={fc2_rate:.4f} (max={MAX_FP_RATE}) | "
            f"lambda={lambda_safety:.1f}"
        )
        ...
        # --- Inner patience: check FC2-rate stagnation at current cost level ---
        ...
        if epochs_without_improvement >= INNER_PATIENCE:
            # FC2-rate stagnated at this cost level → increase cost
            print(f"  -> Stagnation at cost level (no FC2-rate improvement for "
                  f"{INNER_PATIENCE} epochs, best FC2={best_fc2_at_level:.4f}). "
                  f"Increasing lambda {lambda_safety:.1f} -> {lambda_safety + COST_INCREMENT:.1f}")
            print(f"  Val CM:\n{val_cm}")
            lambda_safety += COST_INCREMENT
            cost_step += 1
            # Reset inner patience for new cost level
            best_fc2_at_level = float('inf')
            epochs_without_improvement = 0

    else:
        # while-else: loop finished without break → epoch budget exhausted
        print(f"\n  -> Max fine-tuning epochs ({MAX_FINETUNE_EPOCHS}) reached. "
              f"Constraint not satisfied (FC2-Rate={fc2_rate:.4f}).")
...
def plot_two_phase_training(phase1_df, phase2_df, save_path=None):
    ...
    fig.suptitle("Phase 1: Pre-Training (CE) | Phase 2: Stepwise Cost-Escalation Fine-Tuning (RWWCE)", fontsize=14)
    ...
    ax2 = ax.twinx()
    ax2.plot(phase2_df["epoch"], phase2_df["lambda_safety"], label="lambda", color="tab:gray", marker='x', markersize=3)
    ax.set_ylabel("Average Cost")
...
if __name__ == "__main__":
    ...
    for run in range(1, 8):
        ...
        RESULTS_PATH = paths.BASE_DIR / "artifacts" / "results" / "_random_blobs_balanced" / EXPERIMENT_NAME / f"DATA_SPLIT_STATE_{DATA_SPLIT_STATE}" / "test_runs_constraint_aware_stepwise_WCE"
        ...
        if run == 1:
            ...
            # save constraint aware config to same file as phase 1 config
        ...
        sys.stdout = SaveOutput(RESULTS_PATH / f"RB_results_constraint_aware_stepwise_{run}.txt")

        # ---- Data (same split for both phases) ----
        ...
        pretrain_weights_path = pretrain_weights_dir / f"RB_pretrained_weights_{run}.pth"
        ...
        phase1_history_df.to_csv(
            RESULTS_PATH / f"RB_history_phase1_CE_{run}.csv", index=False)
        torch_NN_evaluation.plot_training_metrics(
            phase1_history_df,
            save_path=RESULTS_PATH / f"RB_figures_phase1_CE_{run}.png",
            Title="Phase 1: Pre-Training (CE)")

        # Phase 2 history
        phase2_history_df.to_csv(
            RESULTS_PATH / f"RB_history_phase2_constraint_{run}.csv", index=False)

        # Combined two-phase plot
        plot_two_phase_training(
            phase1_history_df, phase2_history_df,
            save_path=RESULTS_PATH / f"RB_figures_combined_{run}.png"
        )
        ...
        torch.save(model.state_dict(),
                    finetune_weights_dir / f"RB_constraint_aware_weights_{run}.pth")
