...
if __name__ == "__main__":
    ...
    for run in range(1, 8):
        ...
        DATA_PATH = paths.BASE_DIR / "artifacts" / "data" / "datasets" / "random_blobs_data_balanceds.csv"
        RESULTS_PATH = paths.BASE_DIR / "artifacts" / "results" / "_random_blobs_balanced" / EXPERIMENT_NAME / f"DATA_SPLIT_STATE_{DATA_SPLIT_STATE}" / f"test_runs_RWWCE1_{COST_MATRIX_NAME}" 
        MODEL_PATH = paths.BASE_DIR / "artifacts" / "models" / "model_weights" / "_random_blobs_balanced" / EXPERIMENT_NAME / f"DATA_SPLIT_STATE_{DATA_SPLIT_STATE}" / f"RWWCE1_{COST_MATRIX_NAME}" 
        ...
        sys.stdout = SaveOutput(RESULTS_PATH / f"RB_results_RWWCE_{run}.txt")  # Redirect stdout
        ...
        # # leads to identical results as AECWCE
        # weight_matrix = torch.tensor([[class_weights[0], 0, 0],
        #                                [0, class_weights[1], 0],
        #                                [0, 0, class_weights[2]]], dtype=torch.float32)
        weight_matrix = torch.tensor([[0, 0, 0],
                                       [0, 0, 0],
        ...
        history_df.to_csv(RESULTS_PATH / f"RB_history_RWWCE_{run}.csv", index=False) 
        #df_probs.to_csv(RESULTS_PATH / f"RB_probabilities_RWWCE_{run}.csv", index=False)   

        torch_NN_evaluation.plot_training_metrics(history_df, 
                                    save_path=RESULTS_PATH / f"RB_figures_RWWCE_{run}.png")