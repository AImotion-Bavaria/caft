...
EPOCHS_PER_ITER = 10   #2      # Inner training epochs per Lagrangian update (keep low to preserve pre-trained features)
MAX_OUTER_ITERS = 50           # Maximum number of Lagrangian update steps
MAX_FP_RATE = 0.005            # Max allowable false-class-2 prediction rate
DELTA_LAMBDA = 0.15            # Step size for Lagrangian multiplier update
FINETUNE_LR = 5e-6 #1e-6       # Learning rate for fine-tuning (must be very low to avoid collapse)
STAGNATION_PATIENCE = 10       # Stop if FC2-Rate does not improve for this many outer iters
...
if __name__ == "__main__":
    ...
    for run in range(1, 8):
        ...
        RESULTS_PATH = paths.BASE_DIR / "artifacts" / "results" / "_random_blobs_balanced" / EXPERIMENT_NAME / f"DATA_SPLIT_STATE_{DATA_SPLIT_STATE}" / "test_runs_constraint_aware3"
        ...
        sys.stdout = SaveOutput(RESULTS_PATH / f"RB_results_constraint_aware_{run}.txt")
